...
void solve() {
    ...
    auto get_different_char = [&](int i, int j, char oldc, char up, char left) {
        for (char c : chars) {
            if (c != oldc && c != up && c != left) return c;
        }
        return '?';
    };
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            char up = (i > 0) ? result[i - 1][j] : ' ';
            char left = (j > 0) ? result[i][j - 1] : ' ';
            result[i][j] = get_different_char(i, j, grid[i][j], up, left);
        }
    }
