...
bool is_point_inside_polygon(int px, int py,
...
// code for tower_of_hanoi problem
...
// In-place XOR-FWHT. If inv==true, does the inverse transform (and divides by n).
...
void solve() {
    ...
    for(int b = 0; b < MAXB; b++){
        if(basis[b]) bs.push_back(basis[b]);
    }
    int m = bs.size();
    long long free_count = n - m;
    if(free_count >= 18){
    ...
    long long mult = 1LL << free_count;
    long long t = (k + mult - 1) / mult;
    sort(bs.begin(), bs.end());
    vector<long long> res;
    res.reserve(t);
    res.push_back(0);
    for(long long bvec: bs){
        int sz = res.size();
        vector<long long> temp;
        temp.reserve(sz);
        for(int i = 0; i < sz; i++){
            temp.push_back(res[i] ^ bvec);
        }
        sort(temp.begin(), temp.end());
        vector<long long> merged;
        merged.reserve(t);
        int i = 0, j = 0;
        while((int)merged.size() < t){
            if(i < sz && (j >= sz || res[i] < temp[j])){
    ...
    for(long long v: res){
        long long cntv = mult;
        if(cntv > remaining) cntv = remaining;
        for(long long i = 0; i < cntv; i++){
            if(!first) cout << ' ';
            first = false;
            cout << v;
        }
