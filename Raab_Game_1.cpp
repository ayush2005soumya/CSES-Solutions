...
void solve() {
    ...
    int n, a, b;
        ...
        if(K > 0){
            vector<int> V(K);
            iota(V.begin(), V.end(), T+1);
            vector<int> P_rem(K), Q_rem(K);
            if(a > 0 && b > 0){
                P_rem = V;
                for(int i = 0; i < K; ++i)
                    Q_rem[i] = V[(i + a) % K];
                ...
                P_rem = V; 
                Q_rem = V; 
                reverse(Q_rem.begin(), Q_rem.end());
            }
            else {
                Q_rem = V; 
                P_rem = V; 
                reverse(P_rem.begin(), P_rem.end());
            }
            P.insert(P.end(), P_rem.begin(), P_rem.end());
            Q.insert(Q.end(), Q_rem.begin(), Q_rem.end());
        }
        for(int i = 0; i < n; ++i){
            cout << P[i] << (i+1<n ? ' ' : '\n');
        }
