...
3. Construct the first half of the palindrome using half of each character's frequency.
4. If there's a character with odd frequency, place it in the middle.
5. Construct the second half by reversing the first half.
6. Output the complete palindrome.
...
void solve() {
    ...
    for (char c : chars) {
        first_half += string(freq[c] / 2, c);
    }
    string middle = "";
    if (odd_count == 1) {
        middle = string(freq[odd_char], odd_char);
        middle = middle.substr(freq[odd_char] / 2, 1);
    }
