...
/*algo:
Step 1: Understand when it's possible
Total sum of numbers from 1 to n is:n*(n+1)/2
For equal division, we need:
S must be even
So, if S is odd, it's impossible . print "NO".

Step 2: Constructing the sets (when S is even)
We can use a greedy approach:

Start from the largest number n and work downwards.

Keep adding numbers to set1 until the sum of set1 reaches S/2.

Put remaining numbers in set2.

Why this works:

At each step, we pick the largest number that doesn't exceed the remaining target sum for set1.*/
